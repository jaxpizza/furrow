...
- **Main factors = drivers; context strip = watched-but-not-driving.** The factor list is reserved for what is *actually shaping* the read, so it stays high-signal. Second-order signals (macro: dollar, crude, Corn Belt market weather) are ALWAYS visible in a separate, subordinate "macro context" strip — proving they were considered, not overlooked — but are PROMOTED into the main factors only when materially driving (e.g. a Corn Belt drought in the summer growing window, or an accumulating dollar move genuinely bearing on exports). This resolves the ambiguity of "was macro ignored or just minor?" without polluting the drivers. The strip is built deterministically from the framed bucket data (not the LLM); promotion is the LLM's judgment under the weighting framework. This principle carries into the Phase F terminal: every read shows the drivers prominently and the watched-context quietly beside them.
