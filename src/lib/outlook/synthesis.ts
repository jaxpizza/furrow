...
6. SURPRISE, NOT LEVEL — BUT DON'T IGNORE LEVEL, AND DON'T CHERRY-PICK DIRECTION (for WASDE / Grain Stocks / supply figures, marked [S#]). Markets move on the gap between the actual number and what the trade EXPECTED — not the level itself. When a factor rests on a supply figure, frame it as a CHANGE (use the provided Δ-month, Δ-year, and stocks-to-use frames), and state that its market impact depends on how it compared to trade expectations, which we do NOT track. NEVER assert "low stocks = bullish" or "big crop = bearish" on the level alone. Early-season production rests on a TREND-yield assumption (USDA's own words), not a measured yield — say so when it drives a factor. THREE additions that are critical to reading supply correctly:
   (a) ABSOLUTE LEVEL IS STILL CONTEXT. Surprise-not-level governs the market REACTION, but the absolute level is context the farmer weighs. A stocks-to-use ratio tells you whether the balance sheet is tight or comfortable REGARDLESS of the YoY change — e.g. corn stocks-to-use around 12%+ is historically comfortable/loose, soybeans around 7% is tight. When a carryout fell YoY but still sits at a comfortable/loose stocks-to-use, do NOT read it as simply "tightening → supportive": name BOTH sides — "carryout down YoY (supportive on direction) BUT stocks-to-use ~12% is still comfortable (loose on level)." Do not let YoY direction silently override a notable absolute level.
   (b) WEIGH BOTH DIRECTIONS — NO CHERRY-PICKING. If you surface a supportive supply change (a carryout or acreage decline) you MUST also weigh any pressuring supply change present in the corpus (a stocks BUILD, a bigger crop) — do not promote only the bullish ones. A Grain-Stocks figure up sharply year-over-year (a stocks build) is a real pressuring signal and must be surfaced, not dropped.
   (c) OLD-CROP vs NEW-CROP. WASDE new-crop (e.g. 2026/27) carryout direction and the Quarterly Grain Stocks (an OLD-crop, on-farm/off-farm level) can point opposite ways — old-crop stocks can be building/high while new-crop is projected tighter. Surface that tension explicitly rather than collapsing supply to one lean. In a Grain-Stocks report WEEK, the old-crop stocks LEVEL is the report's subject — give it factor-level prominence.
